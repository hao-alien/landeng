...
const initialState = {connected: false, Settings: {Language: 'en'}}
...
export default function backendReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case constants.BACKEND_MESSAGE_RECEIVED:
      ...
      return assignToEmpty(state, {connected: true, Settings: assignToEmpty(state.Settings, action.status.Message)})
    } else {
